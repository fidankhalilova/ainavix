        ...
        {tools.length < 2 ? (
          ...
          <div className="bg-white rounded-3xl border border-[#E8EAFF] shadow-sm overflow-hidden">
            ...
            {[
              {
                label: "Category",
                render: (t: any) => t.categories?.[0]?.name || "—",
              },
              {
                label: "Pricing",
                render: (t: any) => (
                  <span
                    className={`px-4 py-1 rounded-full text-xs font-semibold capitalize ${PRICING_COLORS[t.pricing?.toLowerCase() || ""] || "bg-gray-100 text-gray-600"}`}
                  >
                    {t.pricing || "—"}
                  </span>
                ),
                    ...
                    <div className="flex gap-0.5">
                      {Array.from({ length: 5 }).map((_, i) => (
                        <Star
                          ...
                          size={18}
                          className={
                            i < Math.round(t.averageRating || 0)
              ...
              },
              {
                label: "Reviews",
                render: (t: any) => (
                  <span className="font-medium">{t.reviewsCount || 0}</span>
                ),
              },
              {
                label: "Short Description",
                render: (t: any) => (
                  <span className="text-sm text-[#1B1464]/70 leading-relaxed">
                    {t.shortDescription || "—"}
                  </span>
                ),
              },
              ...
              <div
                key={label}
                className="grid divide-y divide-[#E8EAFF]"
                style={{
                  gridTemplateColumns: `240px repeat(${tools.length}, 1fr)`,
                }}
              >
                <div className="p-6 pl-8 font-medium text-[#1B1464] border-b border-[#E8EAFF]">
                  {label}
                </div>
                {tools.map((tool) => (
                  <div
                    key={tool._id}
                    className="p-6 text-[#1B1464]/80 border-b border-[#E8EAFF]"
                  >
                    {render(tool)}
                  </div>
                ))}
...
export default function ComparePage() {
  return (
    <Suspense
      fallback={
        <div className="min-h-screen bg-[#F8F9FF] pt-20 flex items-center justify-center">
          <div className="w-10 h-10 rounded-full border-4 border-[#E8EAFF] border-t-[#2E4BC6] animate-spin" />
        </div>
      }
    >
      <ComparePageInner />
