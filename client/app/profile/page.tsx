...
import { Camera, Star, Edit2, Save, X, LogOut, Package, MessageSquare, Plus, Settings, Heart, Lock as LockIcon, Globe, Lock } from 'lucide-react';
...
export default function ProfilePage() {
  ...
  const { data: reviews   = [] } = useReviewsByUser(user ? (user._id as any) : 0, { enabled: !!user?._id });
  const { data: submitted = [] } = useUserSubmittedTools(user ? (user._id as any) : 0, { enabled: !!user?._id });
  ...
  useEffect(() => { if (user) { setBio(user.bio || ''); setUsername(user.username || ''); } }, [user]);

  const handleSave = async () => {
    if (!user) return;
    try { await updateProfile.mutateAsync({ userId: user._id, data: { bio, username } }); await refreshUser(); setEditing(false); toast.success('Profile updated!'); }
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try { await uploadAvatar.mutateAsync({ userId: user._id, file }); await refreshUser(); toast.success('Avatar updated!'); }
  ...
  if (isLoading) return <div className="min-h-screen bg-[#F8F9FF] pt-20 flex items-center justify-center"><div className="w-12 h-12 rounded-full border-4 border-[#E8EAFF] border-t-[#2E4BC6] animate-spin"/></div>;
  if (!isAuthenticated || !user) return (
    <div className="min-h-screen bg-[#F8F9FF] pt-20 flex flex-col items-center justify-center gap-6 px-4 text-center">
      ...
      <h2 className="text-3xl font-bold text-[#1B1464]">Sign in to view your profile</h2>
      <Link href="/auth/login" className="bg-linear-to-r from-[#2E4BC6] to-[#00C2CB] text-white font-semibold px-8 py-3.5 rounded-2xl">Sign In</Link>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#F8F9FF]">
          ...
          <div className="flex flex-col md:flex-row items-center md:items-end gap-6 md:gap-8">
            <div className="relative shrink-0">
              <div className="w-28 h-28 md:w-32 md:h-32 rounded-3xl overflow-hidden border-4 border-white shadow-2xl bg-linear-to-br from-[#4A6DE0] to-[#00C2CB]">
                {avatarUrl ? <Image src={avatarUrl} alt={user.username} width={128} height={128} className="object-cover w-full h-full"/> : <span className="w-full h-full flex items-center justify-center text-white text-5xl md:text-6xl font-bold">{user.username[0].toUpperCase()}</span>}
              </div>
            ...
            <div className="flex-1 text-white text-center md:text-left">
              <div className="flex flex-col md:flex-row md:items-center gap-3 md:gap-4">
                {editing ? <input value={username} onChange={e => setUsername(e.target.value)} className="text-2xl md:text-3xl font-bold bg-white/10 border border-white/30 rounded-2xl px-4 py-1.5 text-white w-full max-w-xs focus:outline-none"/> : <h1 className="text-2xl md:text-3xl font-bold tracking-tight">{user.username}</h1>}
              ...
              {editing ? <textarea value={bio} onChange={e => setBio(e.target.value)} maxLength={280} rows={2} placeholder="Tell the community about yourself..." className="mt-3 w-full max-w-lg bg-white/10 border border-white/30 rounded-2xl px-4 py-2 text-sm text-white placeholder-white/60 focus:outline-none resize-y"/> : <p className="text-white/80 mt-2 max-w-md mx-auto md:mx-0 leading-relaxed text-sm">{user.bio || 'No bio yet. Tell the community about yourself!'}</p>}
            </div>
            <div className="flex flex-col sm:flex-row gap-2 w-full md:w-auto">
              {editing ? (
                <>
                  <button onClick={handleSave} disabled={updateProfile.isPending} className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-white text-[#2E4BC6] font-semibold px-5 py-2 rounded-xl text-sm hover:bg-white/90"><Save size={16}/> Save</button>
                  <button onClick={() => { setEditing(false); setBio(user.bio || ''); setUsername(user.username); }} className="flex-1 sm:flex-none flex items-center justify-center gap-2 border border-white/40 text-white px-5 py-2 rounded-xl text-sm hover:bg-white/10"><X size={16}/> Cancel</button>
                </>
              ) : (
                <>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 -mt-8 relative z-10 pb-20">
                  ...
                  {submitted.slice(0,3).map((tool: Tool) => (
                    <div key={tool._id} className="flex gap-4 items-center">
                      <div className="w-12 h-12 rounded-2xl overflow-hidden bg-[#F8F9FF] border border-[#E8EAFF] shrink-0">
                        {tool.logo?.url && <Image src={getStrapiMedia(tool.logo.url)!} alt={tool.name} width={48} height={48} className="object-contain p-1.5"/>}
                      </div>
        ...
        {tab === 'reviews' && (reviews.length === 0 ? (
          <div className="text-center py-20 bg-white rounded-3xl border border-[#E8EAFF]"><MessageSquare size={70} className="mx-auto text-[#E8EAFF] mb-6"/><p className="text-xl text-[#1B1464]/60">You haven&apos;t written any reviews yet.</p></div>
        ) : (
          <div className="space-y-6">
        ...
        {tab === 'submitted' && (submitted.length === 0 ? (
          <div className="text-center py-20 bg-white rounded-3xl border border-[#E8EAFF]"><Package size={70} className="mx-auto text-[#E8EAFF] mb-6"/><p className="text-xl text-[#1B1464]/60">You haven&apos;t submitted any tools yet.</p><Link href="/tools/submit" className="mt-6 inline-block bg-linear-to-r from-[#2E4BC6] to-[#00C2CB] text-white px-8 py-3 rounded-2xl font-medium">Submit Your First Tool</Link></div>
        ) : (
        ...
        {tab === 'favorites' && (
          <div>
            ...
            {favorites.length === 0 ? (
              <div className="text-center py-20 bg-white rounded-3xl border border-[#E8EAFF]"><Heart size={70} className="mx-auto text-[#E8EAFF] mb-6"/><p className="text-xl text-[#1B1464]/60">Your favorites list is empty</p><Link href="/tools" className="mt-6 inline-block bg-linear-to-r from-[#2E4BC6] to-[#00C2CB] text-white px-8 py-3 rounded-2xl font-medium">Browse Tools</Link></div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {favorites.map((tool: Tool) => (
                  <div key={tool._id} className="bg-white rounded-3xl p-6 border border-[#E8EAFF] relative group">
                    <button onClick={() => removeFromFavorites(tool._id)} className="absolute top-5 right-5 text-red-500 hover:text-red-600 transition-colors"><Heart size={24} fill="currentColor"/></button>
                    <div className="flex gap-4">
